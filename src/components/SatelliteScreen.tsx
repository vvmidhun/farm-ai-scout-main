...
function TileCard({
  ...
  return (
    ...
    >
      ...
      <p className="mt-0.5 line-clamp-2 text-center text-[10px] font-bold leading-tight text-slate-500 sm:text-xs">
        {tile.fact}
      </p>
    ...
    <div className="relative flex h-dvh w-full flex-col overflow-hidden bg-navy">
      ...
      <div className="relative z-10 mx-auto flex h-full w-full max-w-lg flex-col gap-2 px-3 py-3">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
          <div
            className="rounded-2xl border-4 border-dashed border-good/50 bg-good/10 p-2"
            onDragOver={(e) => e.preventDefault()}
            onDrop={onDropZone('keep')}
          >
            <p className="mb-1.5 text-center text-sm font-extrabold uppercase tracking-wider text-good">
          ...
          <div
            className="rounded-2xl border-4 border-dashed border-slate-400/60 bg-slate-200/60 p-2"
            onDragOver={(e) => e.preventDefault()}
            onDrop={onDropZone('shelf')}
          >
            <p className="mb-1.5 text-center text-sm font-extrabold uppercase tracking-wider text-slate-600">
              🗑️ {SATELLITE_SHELF}
            </p>
            {shelfIds.length === 0 ? (
              <p className="py-4 text-center text-xs font-bold text-slate-400">
